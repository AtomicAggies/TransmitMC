...
void loop()
{
  ...
  if (rocket_state == LAUNCH_PAD)
  {
    ...
    if (is_accelerating == false && tot_accel > ACCEL_LAUNCH_THRESHOLD)
    { // first detected acceleration
      accel_start_time = millis();
      is_accelerating = true;
    }
    else if (is_accelerating == true && tot_accel > ACCEL_LAUNCH_THRESHOLD)
    { // acceleration detected in previous loop() iteration // I am still accelerating
      unsigned long curr_time = millis();
      time_accelerating = curr_time - accel_start_time;
      if (time_accelerating > TIME_ACCEL_THRESHOLD)
      { // time accelerating is greater than threshold
        if (activation_state == INACTIVE)
    ...
    { // I have stopped accelerating //this is to avoid false launches
